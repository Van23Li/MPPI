...
class MPPI:
    ...
    def call(self):
        ...
        while np.linalg.norm(self.cur_pos.cpu()-self.pos_goal.cpu(), ord = 2) > bias:
            ## MPPI
    ...
    def get_norm_samples(self, MU_ARR, SIGMA, N_TRAJ):
        ...
        result = result_temp.sample().to(**self.tensor_args)
        return result

    def get_rollout(self, pos_init, u_sampl, dT):
        self.INT_MAT = torch.tril(torch.ones(self.H, self.H)).to(**self.tensor_args)
